...
#define RELAY PORTB.B2
#define TRUE 1
#define FALSE 0

#define CONFIG_ADDR 0x0000

#define ADMIN_ADDRESS 0x00A0
#define USERS_ADDRESS 0X00B0
...
for (i=0;i<sizeof(configString)-1;i++){
    configString[i] = EEPROM_Read(CONFIG_ADDR+i);
}
...
for (i=0;i<10;i++){
admin[i] = EEPROM_Read(ADMIN_ADDRESS+i);
delay_ms(20);
}
...
for (i=0;i<numberOfUsers;i++){
    for (j=0;j<10;j++){
        users[i][j] = EEPROM_Read(i * 10 + USERS_ADDRESS + j);
    }
}
...
for (i=0;i<sizeof(configString)-1;i++){
    EEPROM_Write(CONFIG_ADDR+i,configString[i]);
}
...
if (numberOfUsers == MAXUSERS) return 0;

INTCON.GIE = 0;


for (i=0;i<10;i++){
EEPROM_Write(numberOfUsers * 10 + USERS_ADDRESS + i,*number++);
delay_ms(20);
}
...
num[i] = *number++;
   ...
   if (numberOfUsers == 1){
      strcpy(users[0],"");
      for (j=0;j<10;j++){
          EEPROM_Write(USERS_ADDRESS + j,0xFF);
          delay_ms(20);
      }
      delay_ms(20);
   }else 
   if (strstr(users[numberOfUsers-1],num) != 0){
      ...
      for (j=0;j<10;j++){
          EEPROM_Write(((numberOfUsers-1) * 10) + USERS_ADDRESS + j,0xFF);
          delay_ms(20);
      }
   }else{
       ...
       for (i=0;i<numberOfUsers;i++){
           if (strstr(users[i],num) != 0){
              ...
              for (j=0;j<10;j++){
                  EEPROM_Write((i * 10) + USERS_ADDRESS + j,users[i][j]);
                  delay_ms(20);
              }
...
if (strstr(interruptStreamBuffer,CopyConst2Ram(msg,UNREAD)) != 0)
{
          ...
          if (strstr(interruptStreamBuffer,CopyConst2Ram(msg,ADMINC)) != 0)
          {
                 ...
                 }else 
                 {
                     ...
                     for (i=0;i<10;i++){
                         EEPROM_Write(ADMIN_ADDRESS+i,admin[i]);
                         delay_ms(20);
                     }
         ...
         if ((strstr(admin,phoneBuffer) != 0) && isDataProcessed == FALSE)
         {
               ...
               if (strstr(interruptStreamBuffer,CopyConst2Ram(msg,MODE_)) != 0 )
               {
                  ...
                  char *p1,*p2;
                  int mod;  /*int impulses,mode_2_delay,repeat,wait,mode_1_delay;*/
               ...
               if (strstr(interruptStreamBuffer,CopyConst2Ram(msg,RESET)) != 0 )
               {
                  ...
                  for (i=0;i<numberOfUsers;i++){
                      for (j=0;j<10;j++){
                          EEPROM_Write((i*10) + USERS_ADDRESS + j,0xFF);
                          delay_ms(20);
                      }
                  }
                  for (j=0;j<10;j++){
                       EEPROM_Write(ADMIN_ADDRESS + j,0xFF);
                       delay_ms(20);
                  }
                  impulses = 0;
                  mode_1_delay = 500;
               ...
               if (isConfigChanged == TRUE) SaveConfigToEEPROM();
...
void OpenGate(){
     ...
     switch (mode){
            ...
            case 2:        // MODE 2 OPEN GATE WITH *IMPULSES* *DELAY* between impulses.
            ...
            case 3:      // MODE 3 OPEN GATE UNTIL RING
                 seconds = 0;
